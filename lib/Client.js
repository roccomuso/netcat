...
class Client extends Netcat {
  ...
  send (data, cb) {
    var cbOrHost = (this._protocol === 'tcp') ? (cb || noop) : (cb || '255.255.255.255')
    ...
    if (this._interval) { /* delay */
      setTimeout(function () {
        ...
        debug('Client sent data', data, ((this._protocol === 'udp') ? 'to ' + cbOrHost : ''))
      }, self._interval * 1000)
    } else {
      debug('host', cbOrHost)
      self.client.write(data, cbOrHost)
