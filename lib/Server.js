...
class Server {
  ...
  pipe (outputStream) {
    ...
    this.passThrough.pipe(outputStream)
    return this
  }
...
if (!module.parent){
  const fs = require('fs')
  const nc = new Server()
  nc.port(2389).serve(fs.createReadStream('Server.js')).listen().pipe(fs.createWriteStream('output.txt'))
}
